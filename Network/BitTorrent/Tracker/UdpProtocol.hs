...
packAnnounceResponseGen packPeers ar = do
  case ar of
...
handleUdpRequest sock addr msg = do
        ...
        0 -> do
          ...
          case mresp of
            ...
            Just (respHeader, connId) -> do
              ...
              writeResponse p
        x | x == announceAction -> whenValid addr rh $ do
          whenParses rh announceGetter msg' $ \an -> do
            ...
            let an' = case addr of
                  ...
                  SockAddrInet port addr4 -> case
                    isRfc1918 addr4 of
                      True -> case peerAddr $ anPeer an of
  ...
  where
    ...
    whenParses rh getter msg action = do
