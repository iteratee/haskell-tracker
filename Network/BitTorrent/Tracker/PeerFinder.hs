  ...
  if got >= count -- If there are enough peers in currMap, use those.
    then do
      ...
      let newMap = appendToMap nextMap newAssocs
          ...
          peerlist' =
            RandomPeerList
              {rplCurrent = right, rplNext = newMap, rplReverse = newReverse}
      return (result, peerlist')
    else if M.size nextMap <= count'
           then do
             ...
             let newOldMap = appendToMap right oldAssocs
                 ...
                 newReverse =
                   updateReverseMap (oldAssocs ++ newAssocs) reverseMap
                 peerlist' =
                   RandomPeerList
                     { rplCurrent = newOldMap
                     , rplNext = newMap
...
addPeer p rpl = do
  ...
  case M.lookup k reverseMap of
    Nothing -> do
      ...
      return
        (rpl
           { rplNext = M.insert (r, Just (peerId p)) p nextMap
...
emptyPeerList =
  RandomPeerList {rplCurrent = M.empty, rplNext = M.empty, rplReverse = M.empty}
...
emptyHashRecord = do
  ...
  return HashRecord {hrInet4 = phr4, hrInet6 = phr6}
...
getPeersGen nSeeders nLeechers mphr = do
  ...
  let count = nSeeders + nLeechers
      ...
      (nSeeders', nLeechers') =
        if seedersAvail + leechersAvail <= count
          ...
          else let seedersFloor = min seedersAvail nSeeders
  ...
  putMVar mphr $ phr {phrSeeders = seeders', phrLeechers = leechers'}
